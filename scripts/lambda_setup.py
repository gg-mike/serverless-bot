...
def load_common_files():
    global common_files
    common_files = [common_filepath.split("/")[-1] for common_filepath in glob(f"{src_dir}/common/*.py")]
...
def build_custom_dependencies_for_file(current: str, next_in_hierarchy: list, dep: dict):
    if current in dep:
        ...
        next_req, next_dep = build_custom_dependencies_for_file(new_current, next_in_hierarchy, dep[current])
...
def build_dependencies_for_file(file: str):
    ...
    custom_req, custom_dep = build_custom_dependencies_for_file(current, next_in_hierarchy, dependencies["lambda"])
    ...
    for filepath in glob(f"{src_dir}/lambda/*.py"):
        ...
        for dep in deps:
          if dep not in common_files:
            raise InvalidCommonFileException(f"File '{file}' requires common file '{dep}', but it wasn't found in 'src/common' folder")
